...
static memBlock *quickListStart=NULL;
static int quickListMaxBlockSize=0;
...
//	newBlock = (memBlock *) mmap(0, dataSize+HEADER_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
	...
	if (STRATEGY==4 && dataSize<=quickListMaxBlockSize) {return newBlock;}	// Use quicklists, dont link here
...
void free(void * vPoint) {
	...
	if (!freeBlock->prev && listStart != freeBlock) {return;} // If this is true, the block is part of the quickLists, they have no prev field
...
void *malloc(size_t dataSize) {
	...
	if (STRATEGY==4 && !quickListStart) { // First call
		quickListMaxBlockSize = 8*pow(2,(NRQUICKLISTS-1));
		quickListStart=extendHeap(0);

		int i;
		for (i=1;i<NRQUICKLISTS;i++) {extendHeap(0);} // Add first entries to all lists
	}

	if (STRATEGY==4 && dataSize<=quickListMaxBlockSize) {
		...
		reqBlock = (memBlock*) ( (void*)quickListStart + i*HEADER_SIZE);
